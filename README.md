...
建议在虚拟隔离环境中运行，避免和本机已有 Python 包发生版本冲突。
...
运行前先复制 `.env.example` 为 `.env`，再填入需要使用的 API key。随后启动测试台：
...
It is better to run the project inside an isolated virtual environment, so local Python packages do not conflict with the project dependencies.
...
Before running the script, copy `.env.example` to `.env` and fill in the API keys you want to use. Then start the test console:
